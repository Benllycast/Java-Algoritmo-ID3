...
public class ID3 {
    ...
    private boolean hayDataset;
    private int atributoClase;

    private ID3() {
    }

    public void cargarDataSet( ResultSet dataset ) throws SQLException {

        dataset.beforeFirst();

        ResultSetMetaData meta = dataset.getMetaData();

        /* establecer el numero de atributos */
        numAttributes = meta.getColumnCount();
        ...
        for (int i = 0; i < numAttributes; i++) {
            ...
            attributeNames[i] = meta.getColumnName(i+1);
        }
        ...
        while ( dataset.next() ) {
            ...
            for (int i = 0; i < numAttributes; i++) {
                point.attributes[i] = getSymbolValue(i, dataset.getString(i+1) );
            }
    ...
    public ID3( ResultSet dataset ) throws SQLException {
        cargarDataSet(dataset);
    }

    public ID3( ResultSet dataset, String atribClase ) throws SQLException {
        cargarDataSet(dataset);
        ...
        for (int i = 0; i < meta.getColumnCount(); i++) {
            ...
            if ( col.equals( atribClase ) ) atributoClase = i;
        }
    }
    ...
    public Vector getSubset(Vector data, int attribute, int value) {
        ...
        System.out.println("subset > atrib: " + attributeNames[attribute] + " value: " + domains[attribute].elementAt(value) );
    ...
    public void decomposeNode(TreeNode node) {
        ...
        int numinputattributes = numAttributes;
        ...
        for (int i = 0; i < numinputattributes; i++) {
            ...
            System.out.println("procesando atrib " + attributeNames[i] );
            ...
            System.out.println( averageentropy +  ", entropia del atrib " + attributeNames[i] );

            if (selected == false) {
                selected = true;
                bestEntropy = averageentropy;
                selectedAttribute = i;
                System.out.println("atrib selected");
            } else {
    ...
    public static void mainMY(String[] args) throws Exception {
        ...
        ResultSet data = st.executeQuery( "SELECT * FROM optometria" );
